---
name: "source-command-cb-usage"
description: "코드브레인 활동 — Codex+Codex 토큰 + hook/MCP breakdown + PreToolUse 차단 횟수."
---

# source-command-cb-usage

Use this skill when the user asks to run the migrated source command `cb-usage`.
...
JSON 경로:
- `actual_token_usage.{Codex,codex}.{source, sessions_matched, tokens.{input_tokens, output_tokens, cache_read_input_tokens, cached_input_tokens}}`
...
```
코드브레인 활동
- Codex: {Codex.sessions_matched}세션 · 입력 {input_tokens} / 출력 {output_tokens} / cache_read {cache_read_input_tokens}
...
규칙:
- `Codex.source != "claude_transcript"` 시 첫 줄을 `- Codex: (transcript 없음 — source: <source>)`.
