...
def build_parser() -> argparse.ArgumentParser:
    ...
    queue = sub.add_parser("queue")  # no help= → hidden from --help (still functional)
    ...
    loop = sub.add_parser("loop")  # no help= → hidden from --help (still functional)
    ...
    loopd_p = sub.add_parser("loopd")  # no help= → hidden from --help (still functional)
    ...
    obs_loss = obs_sub.add_parser("loss-accounting")
    obs_loss.add_argument("--json", action="store_true", dest="command_json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "obs" and args.obs_command == "health-summary":
            ...
            emit(payload, as_json=as_json)
            return OK
        if args.command == "obs" and args.obs_command == "loss-accounting":
            from .loss_accounting import summary as loss_summary

            payload = loss_summary(root)
            emit(payload, as_json=as_json)
            return OK if payload.get("ok") is True else GENERIC_ERROR
