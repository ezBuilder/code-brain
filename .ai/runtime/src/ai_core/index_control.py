...
def policy(root: Path, *, max_seconds: int | None = None) -> dict[str, Any]:
    ...
    if not config_path.exists():
        # Search/rebuild is also used as a library by isolated tests and small
        # temporary repos. Preserve the historical safe-default behavior when
        # no Code Brain config has been installed. A present-but-invalid config
...
def progress_status(root: Path, *, effective_policy: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    disabled = current_policy.get("ok") is True and current_policy.get("enabled") is not True
    suppressed_by_policy = bool(disabled and valid)
    ok = bool(
        suppressed_by_policy
        or (not stalled and not orphaned and not failed and (state != "skipped" or skipped_ok))
    )
    result = {
        "ok": ok,
        "bounded": True,
...
class IndexProgress:
    ...
    def checkpoint(self) -> None:
        """Check wall-clock budget while traversing non-file entries."""
        self._check_time()
        self.heartbeat()
