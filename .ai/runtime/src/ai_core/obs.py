...
def write_log(root: Path, level: str, event: str, payload: dict[str, Any]) -> dict[str, Any]:
    record = {
        ...
        "level": level,
        "event": event,
        "payload": _bounded_log_payload(payload),
    }
    path = log_path(root)
...
def metrics(root: Path, *, include_usage: bool = True) -> dict[str, Any]:
    from .retrieval_observation import runtime_summary as retrieval_runtime_summary
    ...
    return {
        ...
        "search": search,
        "retrieval": retrieval_runtime_summary(root),
