...
def is_active_for(root: Path) -> bool:
    ...
    Activation NEVER triggers a download (same wire contract as
    embedding.is_active_for). The pre-006 auto-install here was doubly broken:
    it spawned `ai reranker install`, a command that was never registered, so
    the child died in argparse while the TTL lock made it retry every hour and
    left a lockfile with no artifacts behind. Installs happen only via the now
    real `ai reranker install`.

    Decision tree:
      AI_SEARCH_RERANK=1/true   → on iff deps importable
      AI_SEARCH_RERANK=0/false  → off (explicit opt-out)
      AI_SEARCH_RERANK unset    → on iff deps + model already present
    """
