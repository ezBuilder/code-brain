...
def prune_directory(
    ...
    *,
    ...
    if dry_run:
        projected_expired = [
            path.name
            for path, state in remaining
            if float(state.st_mtime) < cutoff
        ]
        projected_violations: list[str] = []
        if projected_expired:
            projected_violations.append(f"expired={len(projected_expired)}")
        if len(remaining) > file_limit:
            projected_violations.append(f"files={len(remaining)}>{file_limit}")
        if remaining_bytes > byte_limit:
            projected_violations.append(f"bytes={remaining_bytes}>{byte_limit}")
        if errors:
            projected_violations.append(f"unsafe={len(errors)}")
        after = {
            **after,
            "ok": not projected_violations,
            "count": len(remaining),
            "bytes": remaining_bytes,
            "expired": projected_expired,
            "violations": projected_violations,
            "projected": True,
            "projected_count": len(remaining),
            "projected_bytes": remaining_bytes,
            "actual_count": after.get("count", 0),
            "actual_bytes": after.get("bytes", 0),
        }
    return {
