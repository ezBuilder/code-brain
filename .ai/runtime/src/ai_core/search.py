...
def drop_schema(conn: sqlite3.Connection) -> None:
    # sqlite3.Connection.executescript() commits any open transaction before
    # executing. Rebuild relies on DDL being part of BEGIN IMMEDIATE so a scan
    # limit or crash can roll back to the last good index. Execute each fixed
    # statement separately to preserve that transaction boundary.
    for table in (
        "chunks_fts",
        "chunk_meta",
        "summaries",
...
def create_schema(conn: sqlite3.Connection) -> None:
    statements = (
        ...
        """create virtual table if not exists chunks_fts using fts5(
          path, content, content='', contentless_delete=1,
...
def query(root: Path, text: str, *, limit: int = 5, evidence_source: str | None = None) -> dict[str, Any]:
    ...
    if effective_policy.get("ok") is not True:
        raise RuntimeError(
            ...
            + "; ".join(str(item) for item in effective_policy.get("errors", [])[:5])
        )
    if effective_policy.get("enabled") is not True and not db_path(root).exists():
        payload = {
            "ok": True,
            "query": text,
            "retrieval_policy": "disabled-no-index",
            "recommended_retrieval_policy": "explicit-force-rebuild",
            "results": [],
            "rg_fallback": False,
            "dense_rerank": False,
            "auto_refresh": auto_refresh,
            "reason": "indexing_disabled_no_existing_index",
        }
        if evidence_source:
            _record_evidence_candidates(root, query_text=text, results=[], source=evidence_source)
        return payload
...
def _auto_refresh_if_stale(root: Path) -> dict[str, Any]:
    ...
    metadata_probe = IndexProgress(
        root=root,
        operation="auto_refresh_metadata_probe",
        effective_policy=effective_policy,
        persist=False,
    )
    metadata_probe.begin()
    try:
        source_mtime = max(
            (
                state.st_mtime
                for _path, state in iter_text_file_states(
                    root,
                    use_cache=False,
                    update_cache=False,
                    progress=metadata_probe,
                )
            ),
            default=0.0,
        )
    ...
    if not (
        source_mtime >= db_mtime
        or 0 <= db_mtime - source_mtime <= MTIME_STALE_GRACE_SECONDS
    ):
    ...
    if not hash_status.get("ok") and hash_status.get("reason") not in {"current"}:
        ...
        if hash_status.get("reason") == "legacy_schema":
            # A query must never silently destroy/migrate an old index. Keep
            # the explicit `ai index rebuild` operator boundary and let the
            # subsequent schema check return the actionable legacy error.
            return {
                "enabled": True,
                "rebuilt": False,
...
def index_hash_status(
    ...
    *,
    ...
    try:
        with connect(root) as conn:
            init_schema(conn)
            indexed: dict[str, tuple[str, tuple[int, int, int] | None]] = {}
            for row in conn.execute(
                """
                ...
                next_count = len(indexed) + 1
                if next_count > int(effective_policy["max_files"]):
                    raise IndexScanLimit("max_files", next_count, effective_policy["max_files"])
                probe.checkpoint()
...
def candidate_files(
    ...
    *,
    ...
    if progress is not None:
        # Candidate-cache validation walks the directory tree and predates the
        # bounded progress contract. Rebuild/freshness operations therefore use
        # streaming Git discovery (or the bounded fallback) instead.
        use_cache = False
        update_cache = False
    if use_cache:
