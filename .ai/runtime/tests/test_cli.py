...
def test_cb_upgrade_command_assets_exist() -> None:
    ...
    assert "Hook registration is not hook activation." in readme
...
def test_code_index_uses_git_baseline_and_skips_dependencies(tmp_path: Path) -> None:
    ...
    # Schema v9 indexes untracked, non-ignored git source files (git ls-files --others);
...
def test_code_index_migrates_v11_reference_schema_without_target_leaf(tmp_path: Path) -> None:
    repo = copy_repo(tmp_path)
    db = repo / ".ai" / "cache" / "code.sqlite"
    db.parent.mkdir(parents=True, exist_ok=True)
    with sqlite3.connect(db) as conn:
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    with sqlite3.connect(db) as conn:
        columns = [row[1] for row in conn.execute("pragma table_info(code_references)").fetchall()]
        user_version = conn.execute("pragma user_version").fetchone()[0]
    assert user_version == 11
    assert "target_leaf" in columns
...
def test_index_rebuild_and_code_query(tmp_path: Path) -> None:
    ...
    assert all(
        not str(item["snippet"]).startswith("[stale index: source unavailable")
...
def test_bootstrap_idempotency_integration_invariants() -> None:
    ...
    assert "scripts/run-observed-command.sh bootstrap-pytest" in bootstrap
...
def test_upgrade_latest_dry_run_reports_public_repo_defaults() -> None:
    ...
    activation = payload["codex_hook_activation"]
    assert activation["required"] is True
    assert activation["project_trust_required"] is True
    assert activation["review_command"] == "/hooks"
...
def test_upgrade_latest_from_local_repo_updates_install_manifest(tmp_path: Path) -> None:
    ...
    assert manifest["codex_hook_activation_required"] is True
    assert manifest["codex_project_trust_required"] is True
    assert manifest["codex_hook_review_command"] == "/hooks"
    assert manifest["codex_hook_reapproval_after_changes"] is True
    assert payload["codex_hook_activation"]["review_command"] == "/hooks"
