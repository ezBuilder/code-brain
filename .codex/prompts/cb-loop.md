...
Modes:
- If `$ARGUMENTS` is empty: act as orchestrator. Run `.ai/bin/ai loop claim --orchestrator-id codex-loop --agent codex --json`.
...
- Otherwise: act as producer. Run `.ai/bin/ai loop submit --source-agent codex --target-agent claude --role worker --priority P1 --interval-seconds 300 --text "$ARGUMENTS" --json`.
