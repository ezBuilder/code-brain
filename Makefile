...
help:
	@printf '%s\n' \
		...
		'  make stress-bounds     Stress bounded transcript and sandbox diagnostics' \
...
test:
	./scripts/run-observed-command.sh make-test -- env -u CI -u GITHUB_ACTIONS -u GITLAB_CI -u AI_CI uv run --project .ai/runtime python -m pytest .ai/runtime/tests

eval:
	uv run --project .ai/runtime python .ai/evals/run.py --axis precall_routing --axis context_budget --axis tool_discovery --axis autoresearch_retrieval --axis code_retrieval --axis code_navigation --axis memory_retrieval --wired --strict --require-complete
...
stress-bounds:
	uv run --project .ai/runtime python scripts/stress-bounds.py
