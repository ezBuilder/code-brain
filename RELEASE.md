...
- `uv.lock` is current for `.ai/runtime` through `uv lock --check --project .ai/runtime`.
...
- Make targets wrap common operator flows, including `make env-check`, `make lock-check`, `make lint`, `make quick`, `make verify-artifacts`, and `make release-gate`.
