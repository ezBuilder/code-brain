...
- Session startup is covered by `ai session start`; Mac/VPS agents can begin from the GitHub baseline and regenerate local cache/index on demand.
- Git hooks are installed through `bootstrap.sh` or `make install-hooks`; `.githooks/post-merge` and `.githooks/post-checkout` run dry-run startup checks after Git updates.
...
- Make targets wrap common operator flows, including `make env-check`, `make lockfile-check`, `make lock-check`, `make session-start`, `make install-hooks`, `make lint`, `make quick`, `make verify-artifacts`, and `make release-gate`.
