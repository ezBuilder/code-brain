...
| Strict doctor | 현재 체크아웃에서 source-derived 29개 check를 강제 | 배포 상태와 `secret_scan`·`index_freshness` 같은 건강 상태를 분리 보고 |
...
4. strict doctor는 프로젝트 건강과 자율 라운드 완결성을 검증한다. 주입 컨텍스트 예산·global-kit 소스와 설치물의 통합 drift는 아직 별도 후속 과제다.
