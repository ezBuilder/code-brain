...
| Strict doctor | 현재 체크아웃에서 27개 check가 모두 통과 | 배포 상태와 `secret_scan`·`index_freshness` 같은 건강 상태를 분리 보고 |
| 결정론 평가 | `make eval`이 5개 축, 23/23 케이스를 통과 | 검색·라우팅 변경은 자체 코퍼스 회귀로 판정 |
...
현재 `make eval`의 강제 축은 `precall_routing`, `context_budget`, `tool_discovery`, `autoresearch_retrieval`, `code_retrieval`이다. `decision_logging`은 실제 prompt-to-memory production path가 없어 명시적으로 미지원 상태다. 미지원 축을 성공으로 계산하지 않는다.
...
3. 연구 후보, todo/plan, loop request, diff, acceptance, verdict evidence, 다음 라운드를 하나의 round ID로 묶는 강제 계약이 없다.
4. strict doctor는 프로젝트 건강을 잘 검증하지만, 자율 라운드 완결성·주입 컨텍스트 예산·global-kit 소스와 설치물의 통합 drift를 하나의 계층형 결과로 보여주지 않는다.
...
권장 저장 위치는 durable output인 `.ai/outputs/`이며, todo·plan·loop ledger에는 같은 `round_id`를 source/tag로 기록한다. runtime 구현 전까지는 Markdown round report를 canonical record로 사용하고, 서로 다른 ledger의 ID를 문서에 명시적으로 연결한다.
...
현재 27개 strict check를 유지하면서 다음 check를 우선 검토한다.

1. `autonomous_round_completeness`: round/task/acceptance/evidence 연결과 미종결 P0/P1 탐지
