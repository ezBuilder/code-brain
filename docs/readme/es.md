...
v0.7.0 añade olvido demostrable y recall medido: `ai memory forget` borra en duro una decisión/fallo (tombstone + compactación + recibo de borrado) para que ningún camino de lectura — incluida la inyección de SessionStart — pueda resurgirla; `ai memory forget-note` hace lo mismo con las notas de sesión. Un eje de evaluación `memory_retrieval` controla la calidad del recall en `make eval` (los registros caducados, refutados o con tombstone nunca deben rankear), y las descargas de modelos solo ocurren mediante `ai embedding install` / `ai reranker install` explícitos — los caminos de consulta nunca tocan la red. v0.7.1 endurece el contrato del índice: los caminos de lectura nunca migran un índice legado estructural — la consulta falla de forma determinista con la remediación explícita `ai index rebuild`, independiente del mtime y con crecimiento de almacenamiento cero ante fallos repetidos.
