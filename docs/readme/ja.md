...
Code Brain は、本格的な AI コーディングエージェントのためのリポジトリローカルなインフラストラクチャです。Claude Code、Codex CLI、Google Antigravity、Kiro に対して、同じプロジェクトメモリ、BM25 コード検索、フックポリシー、監査証跡、そしてアップグレード経路を、ひとつのワークスペース内で提供します（MCP の公開範囲はホストごとに異なります）。
...
MCP の引数はハンドラ実行前に検証されます。必須フィールドは必ず存在しなければならず、必須の文字列は空文字や空白であってはなりません — `tools/list` がすべての必須文字列に `minLength: 1` を公開するため、行儀のよいクライアントはローカルで呼び出しを拒否できます。拒否メッセージはスキーマで宣言されたフィールド名のみを示し(呼び出し側のテキストは決して含みません)、同一の呼び出しが3回連続で拒否されるとエラーは明示的な停止指示に格上げされます。独自のループ検出を持たないクライアントでも、同じ不正な呼び出しを回し続けることはできません。
