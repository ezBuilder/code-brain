...
- Memory DAG 엣지(opt-in): `record_decision`에 contradicts/derives_from/expires_at 관계 추가, 만료 결정은 surface 제외(include_expired로 우회)
- MCP resources(opt-in `AI_MCP_RESOURCES`): plan/report/handoff/session을 `codebrain://…` 읽기전용 리소스로 노출(resources/list·read, 경로탈출 차단·redact)
- cAST 구조 청킹(opt-in `AI_AST_CHUNK`, Python 파일): stdlib ast 재귀분할+형제병합으로 검색 recall 향상(기본 OFF, 무측정 default 변경 없음)
