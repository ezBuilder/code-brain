...
def python_check() -> dict[str, object]:
    ...
    result: dict[str, object] = {
        "command": "uv python",
        "required": True,
        "path": "uv run --project .ai/runtime python" if uv else None,
        "ok": False,
        "version": None,
        "minimum": "3.11",
    }
    if not uv:
        result["error"] = "uv is required before Python runtime can be resolved"
        return result
    try:
        output = subprocess.check_output(
            [uv, "run", "--project", ".ai/runtime", "python", "-c", "import sys; print(sys.version.split()[0])"],
            cwd=root,
            env=command_env(),
            text=True,
